function showCharityTab(event, tabId) {
  document
    .querySelectorAll(".charity-tab-content")
    .forEach((tab) => tab.classList.remove("active"));
  document.getElementById(tabId).classList.add("active");
  document
    .querySelectorAll(".charity-tab-button")
    .forEach((btn) => btn.classList.remove("active"));
  event.target.classList.add("active");

  if (tabId === "charity-tab1" && window.charitySwiper1)
    setTimeout(() => window.charitySwiper1.update(), 200);
  if (tabId === "charity-tab2" && window.charitySwiper2)
    setTimeout(() => window.charitySwiper2.update(), 200);
  if (tabId === "charity-tab-qura" && window.charitySwiper3)
    setTimeout(() => window.charitySwiper3.update(), 200);
  if (tabId === "charity-tab3" && window.charitySwiper4)
    setTimeout(() => window.charitySwiper4.update(), 200);
}
...
window.addEventListener("load", function () {
  new Swiper(".aboutSwiper", {
    ...
    navigation: {
      nextEl:
        ".aboutSwiper .swiper-button-next" /* Target navigation inside this swiper */,
